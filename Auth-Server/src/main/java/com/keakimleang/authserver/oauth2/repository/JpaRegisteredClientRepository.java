...
import com.keakimleang.authserver.oauth2.service.OAuth2Util;
...
// This is registered as spring bean in OAuth2RegisteredClientConfig. No need to use @component
@Slf4j
public class JpaRegisteredClientRepository implements RegisteredClientRepository {
    ...
    public RegisteredClient findByClientId(String clientId) {
        ...
        var client = this.oauth2RegisteredClientRepository.findByClientId(clientId).map(this::toObject).orElse(null);
        if (client == null) {
            log.warn("No client found for id {}. It may already registered in OAuth2Client.", clientId);
        }
        return client;
    }

    private RegisteredClient toObject(OAuth2RegisteredClient oauth2RegisteredClient) {
        log.info("Registering client {} with redirect URIs {}", oauth2RegisteredClient.getClientId(),
