...
public class OAuth2RegisteredClientService {
    ...
    public OAuth2RegisteredClientService(OAuth2TokenSettingsService oauth2TokenSettingsService,
    ...
    public List<RegisteredClient> getOAuth2RegisteredClient() {
        ...
        log.info("Found {} OAuth2 clients to register: {}", oauth2Clients.size(), oauth2Clients);
    ...
    private RegisteredClient getRegisteredClient(OAuth2Client client) {
        ...
        ClientSettings clientSettings = OAuth2Util.getClientSettings(clientSetting);
    ...
    private Set<AuthorizationGrantType> convertAuthorizationGrantTypes(String authorizationGrantType) {
        return Stream.of(authorizationGrantType.split(",", -1))
                .map(OAuth2Util::resolveAuthorizationGrantType)
                .collect(Collectors.toSet());
    }

    private Set<ClientAuthenticationMethod> convertClientAuthenticationMethods(String clientAuthenticationMethod) {
        return Stream.of(clientAuthenticationMethod.split(",", -1))
                .map(OAuth2Util::resolveClientAuthenticationMethod)
                .collect(Collectors.toSet());
    }
